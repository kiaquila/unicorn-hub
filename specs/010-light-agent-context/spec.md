...
- FR-005: Update local preflight and PR Guard so context-budget validation runs before publish, against local committed changes as committed blobs, against committed PR diffs in PR Guard, and against staged, unstaged, and untracked local spec changes from their matching snapshots.
